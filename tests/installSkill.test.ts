...
test("install-skill.sh can install into both supported agent environments", () => {
  ...
  expect(result.status).toBe(0);
  expect(
    existsSync(path.join(claudeHome, "skills/redline-review/SKILL.md")),
  ).toBe(true);
  expect(
    existsSync(path.join(codexHome, "skills/redline-review/SKILL.md")),
  ).toBe(true);
});
...
test("redline install-skill forwards to the installer", () => {
  ...
  const result = spawnSync(
    process.execPath,
    ["run", CLI, "install-skill", "--agent", "codex"],
    {
  ...
  expect(result.status).toBe(0);
  expect(existsSync(path.join(tmp, "skills/redline-review/SKILL.md"))).toBe(
    true,
  );
});
...
test("install-skill.sh fails if the source SKILL.md has no placeholder", () => {
  ...
  spawnSync("cp", [
    path.join(REPO_ROOT, "skills/redline-review/SKILL.md"),
    fakeSkillDir,
  ]);
  const skillFile = path.join(fakeSkillDir, "SKILL.md");
  writeFileSync(
    skillFile,
    readFileSync(skillFile, "utf-8").replaceAll("__REDLINE_BIN__", "redline"),
  );

  // Stage src/cli.ts and the script so REPO_ROOT resolution works.
  spawnSync("mkdir", [
    "-p",
    path.join(fakeRepo, "src"),
    path.join(fakeRepo, "scripts"),
  ]);
  spawnSync("cp", [
    path.join(REPO_ROOT, "src/cli.ts"),
    path.join(fakeRepo, "src/cli.ts"),
  ]);
  ...
  const result = runInstall({
    claudeHome: tmp,
    agent: "claude",
    scriptPath: path.join(fakeRepo, "scripts/install-skill.sh"),
  });
