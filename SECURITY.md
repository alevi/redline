...
- **Adversarial Markdown trying to manipulate the model via prompt injection.** The selected local provider CLI subprocess receives the document text and comment thread as input; carefully crafted content can attempt to redirect the model. Treat the model's output the same way you'd treat any other AI output you didn't fully verify.
...
- Run Redline only on documents from sources you trust. The HTML render is sanitized; the content sent to the selected local provider CLI subprocess is not.
