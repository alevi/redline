...
export function initSSE(): void {
  ...
  (function connectEvents() {
    ...
    on("responder-unavailable", (e) => {
      let reason = "The caller is no longer connected.";
      try {
        const data = JSON.parse(e.data);
        if (data.reason) reason = data.reason;
      } catch {}
      ...
      if (el) {
        el.textContent =
          state.responderMode === "caller" ? "Caller offline" : "Agent offline";
        el.setAttribute("title", reason);
        el.removeAttribute("hidden");
      }
    });
    on("responder-available", () => {
      const el = document.getElementById("agent-status");
      if (el) el.setAttribute("hidden", "");
    });
    on("responder-changed", () => window.location.reload());
    on("finished", () => {
